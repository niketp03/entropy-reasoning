...
def main():
    ...
    if args.resume_from_checkpoint:
        ...
        accelerator.print(f"Loading from checkpoint: {checkpoint_path}")
        
        # Load model weights first
        model = LayerLoopingModel.from_pretrained(
            args.model_name_or_path,  # Start with base model
            n=args.n,
            m=args.m,
            max_loop_count=args.max_loop_count
        )
        
        # Then load accelerator state
        try:
            ...
            if checkpoint_name.startswith("checkpoint-"):
                ...
                completed_steps = completed_steps % num_update_steps_per_epoch
                
            accelerator.print(f"Successfully loaded accelerator state from: {checkpoint_path}")
            accelerator.print(f"Starting epoch: {starting_epoch}")
            accelerator.print(f"Completed steps: {completed_steps}")
        except Exception as e:
            accelerator.print(f"Warning: Could not load accelerator state: {str(e)}")
            starting_epoch = 0
            completed_steps = 0
    else:
    ...
    # Initialize starting epoch and steps
    starting_epoch = 0
    completed_steps = 0
